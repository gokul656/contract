...
async function main() {
  ...
  const programId = new PublicKey(
    "3sisv9ewdVuJRVqKtNfAaEJWo56DtRySna6mtG1fiPLs"
  );

  // Define the data structure for the instruction
  const voteParamsLayout = struct([str("id"), vec(str("options"), "options")]);
  ...
  let layoutFields = Object.assign(
    ...
    { id: "yazh", options: ["btc", "sol"] }
  );

  voteParamsLayout.encode({ id: "yazh", options: ["btc", "sol"] }, dataBuffer);

  const instructionBuffer = dataBuffer.slice(
    0,
    voteParamsLayout.getSpan(dataBuffer)
  );
